...
/**
 * UpdateSchoolCodeAndCalculationTargetApiController のテスト.
 */
@ExtendWith(SpringExtension::class)
@SpringBootTest
@AutoConfigureMockMvc
class UpdateSchoolCodeAndCalculationTargetApiControllerTest {
    ...
    @MockBean
    private lateinit var updateSchoolCodeAndCalculationTargetUseCase: UpdateSchoolCodeAndCalculationTargetUseCase
    ...
    fun testUpdateSchoolCodeAndCalculationTarget() {
        ...
        val scheduleCode = "SCHEDULE-0001"

        // execution
        mockMvc.perform(post("/api/schedules/{schedule_code}/_update-school-code-and-calculation-target", scheduleCode)
        ...
        verify(updateSchoolCodeAndCalculationTargetUseCase).update(parameter.toParameter(scheduleCode, user.userCode))
    }

    @Test
    @WithMockUser
    fun testUpdateSchoolCodeAndCalculationTarget_BadRequest_400() {
        // setup
        val authorizedClientRegistrationId = "google"
        val principalName = "abcd-000A-0001"
        whenever(oAuth2AuthenticationToken.authorizedClientRegistrationId).thenReturn(authorizedClientRegistrationId)
        ...
        whenever(updateSchoolCodeAndCalculationTargetUseCase.update(any())).thenThrow(exception)
        ...
        // execution
        mockMvc.perform(post("/api/schedules/{schedule_code}/_update-school-code-and-calculation-target", "fuga")
            .contentType(MediaType.APPLICATION_JSON)
            .with(SecurityMockMvcRequestPostProcessors.csrf())
            .content(content))
            // verify
    ...
    @Test
    @WithMockUser
    fun testUpdateSchoolCodeAndCalculationTarget_NotFound_404() {
        // setup
        val authorizedClientRegistrationId = "google"
        val principalName = "abcd-000A-0001"
        whenever(oAuth2AuthenticationToken.authorizedClientRegistrationId).thenReturn(authorizedClientRegistrationId)
    ...
    @Test
    @WithMockUser
    fun testUpdateSchoolCodeAndCalculationTarget_NotCsrfToken_403() {
        // setup
        val parameter = UpdateSchoolCodeAndCalculationTargetApiParameterFixtures.create()
        val mapper = ObjectMapper()
        val content = mapper.writeValueAsString(parameter)
