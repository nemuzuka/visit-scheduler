...
/**
 * CreateScheduleApiController のテスト.
 */
@ExtendWith(SpringExtension::class)
@SpringBootTest
@AutoConfigureMockMvc
class CreateScheduleApiControllerTest {
    ...
    fun testCreateSchedule() {
        ...
        whenever(createScheduleUseCase.createSchedule(any())).thenReturn(createdSchedule)
        ...
        mockMvc.perform(post("/api/schedules")
            ...
            .andExpect(jsonPath("schedule_code").value(`is`(createdSchedule.scheduleCode)))

        verify(getUserUseCase).getUser(authorizedClientRegistrationId, principalName)
        verify(createScheduleUseCase).createSchedule(parameter.toParameter(user.userCode))
    }

    @Test
    @WithMockUser
    fun testCreateSchedule_ConflictScheduleCode_409() {
        // setup
        val authorizedClientRegistrationId = "google"
        val principalName = "abcd-000A-0001"
        whenever(oAuth2AuthenticationToken.authorizedClientRegistrationId).thenReturn(authorizedClientRegistrationId)
        ...
        whenever(createScheduleUseCase.createSchedule(any())).thenThrow(exception)
    ...
    @Test
    @WithMockUser
    fun testCreateSchedule_BadRequest_400() {
        // setup
        val authorizedClientRegistrationId = "google"
        val principalName = "abcd-000A-0001"
        whenever(oAuth2AuthenticationToken.authorizedClientRegistrationId).thenReturn(authorizedClientRegistrationId)
    ...
    @Test
    @WithMockUser
    fun testCreateSchedule_NotCsrfToken_403() {
        // setup
        val parameter = CreateScheduleApiParameterFixtures.create()
        val mapper = ObjectMapper()
        val content = mapper.writeValueAsString(parameter)
